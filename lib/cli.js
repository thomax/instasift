...
function storeAllProfiles(profile) {
  var index = indexOfProfile(profile);
  if (index > -1) {
    _allProfiles2.default[index] = profile;
  } else {
    _allProfiles2.default.push(profile);
  }
  (0, _fs.writeFileSync)(dataDir + '/allProfiles.json', JSON.stringify(_allProfiles2.default, null, 2));
}

function indexOfProfile(profile) {
  for (var i = 0; i < _allProfiles2.default.length; i++) {
    if (profile.username == _allProfiles2.default[i].username) {
      return i;
    }
  }
  return -1;
